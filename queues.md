...
In this example, note that we were able to pass an [Eloquent model](/docs/{{language}}/{{version}}
...
The `handle` method is called when the job is processed by the queue. Note that we are able to type-hint dependencies on the `handle` method of the job. The Laravel [service container](/docs/{{language}}/{{version}}
...
> {tip} The `dispatch` helper provides the convenience of a short, globally available function, while also being extremely easy to test. Check out the Laravel [testing documentation](/docs/{{language}}/{{version}}
...
Using the `before` and `after` methods on the `Queue` [facade](/docs/{{language}}/{{version}}
-/facades), you may specify callbacks to be executed before or after a queued job is processed. These callbacks are a great opportunity to perform additional logging or increment statistics for a dashboard. Typically, you should call these methods from a [service provider](/docs/{{language}}/{{version}}
-/providers). For example, we may use the `AppServiceProvider` that is included with Laravel:
