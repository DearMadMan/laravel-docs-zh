...
在上面的自理中，`sidebar` 挂件利用 `@@parent` 指令追加布局中的 sidebar 部分的内容，如果不使用怎会覆盖掉布局中的这部分。`@@parent` 指令会在视图被渲染时替换为布局中的内容。

当然，就像原生的 PHP 视图一样，Blade 视图也可以通过使用全局帮助函数 `view` 方法来返回:
...
你可以使用花括号双 `{}` 来在视图中展示传递到视图中的变量，例如，你定义了下面的路由：
...
你也可以使用 `@hasSection` 指令来判断提供给布局的挂件中是否包含了内容:
...
> **注意** 你应该在 Blade 视图中避免使用 `__DIR__`` 和 `__FILE__` 常量，因为它们会被解析为视图缓存所在的位置。（还记得吗，Blade 模板是编译为 PHP 原生代码作为视图缓存)

#### 为集合渲染视图

你可以使用 Blade 的 `@each` 指令在一行中合并引入多个视图：
...
Blade 也允许你在其它视图或布局中已经命名的堆栈中压入数据:
...
你可以使用 `@inject` 指令来从 [服务容器](/docs/{{language}}/{{version}}/container) 中取回服务。该指令的第一个参数将作为所取回服务存放的变量名，而第二个参数是你想要在服务容器中取回的类或接口名称：
...
下面的例子中创建了一个 `@datetime($var)` 指令来格式化 `$var`：
...
就如你所看到的，在这个指令中我们使用了 Laravel 的 `with` 帮助方法，它只是简单的返回一个所提供的对象或值，这允许你方便的链式调用方法。最终该指令生成了如下的 PHP 代码：
