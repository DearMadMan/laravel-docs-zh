...
在上面的例子中，`UserController` 需要从数据源中获取到用户的数据，所以我们将 `注入` 一个服务，这个服务可以检索出用户。在这种场景下，我们的 `UserRepository` 更像是使用 [Eloquent](/{{language}}/{{version}}/eloquent) 来从数据库中检索出用户信息。但是不管怎么样，由于存储库的被注入，我们可以非常轻松的换成其它的实现。我们也可以非常轻松的去模拟数据，或者是在进行测试时创建一个假的 `UserRepository` 来使用。
...
几乎所有的服务容器的绑定都是在 [服务提供者](/{{language}}/{{version}}/providers) 中被注册的，所有下面的例子都是在这个场景下进行演示的。
...
有时候你可能会有实现了同一个接口的两个类，而且你想要在不同的场景下注入不同的实现，比如说，两个控制器可以各自依赖 `Illuminate\Contracts\Filesystem\Filesystem` [contract](/{{language}}/{{version}}/contracts) 的不同实现。Laravel 提供了一种简单流利的接口来定义这种行为：
...
另外，也是最重要的，你也可以在类的构造函数中简单的使用‘类型提示’来从容器中解析依赖。[控制器](/{{language}}/{{version}}/controllers)，[事件监听器](/{{language}}/{{version}}/events)，[队列任务](/{{language}}/{{version}}/queues)，[中间件](/{{language}}/{{version}}/middleware)等都支持这种方式。在实践中，这是大部分对象从容器中解析的方式。
