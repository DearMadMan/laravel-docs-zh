...
You may interact with Redis by calling various methods on the `Redis` [facade](/docs/{{language}}/{{version}}/facades). The `Redis` facade supports dynamic methods, meaning you may call any [Redis command](http://redis.io/commands) on the facade and the command will be passed directly to Redis. In this example, we will call the `GET` command on Redis by calling the `get` method on the `Redis` facade:
...
First, let's setup a listener on a channel via Redis using the `subscribe` method. We will place this method call within an [Artisan command](/docs/{{language}}/{{version}}/artisan) since calling the `subscribe` method begins a long-running process:
